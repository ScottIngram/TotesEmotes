...
Group Types
  ...
  Inline Groups
    ...
    - When a group is displayed inline, all descendants will also be inline mtotesers of the group
...
--Option mtoteser type definitions, used to decide how to access it

--Is the mtoteser Inherited from parent options
local isInherited = {
	set = true,
	get = true,
	func = true,
...
--gets the value for a mtoteser that could be a function
--function refs are called with an info arg
--every other type is returned
local function GetOptionsMtoteserValue(mtotesername, option, options, path, appName, ...)
	--get definition for the mtoteser
	local inherits = isInherited[mtotesername]


	--get the mtoteser of the option, traversing the tree if it can be inherited
	local mtoteser

	if inherits then
		local group = options
		if group[mtotesername] ~= nil then
			mtoteser = group[mtotesername]
		end
		for i = 1, #path do
		...
		mtoteser = option[mtotesername]
	end

	--check if we need to call a functon, or if we have a literal value
	if ( not allIsLiteral[mtotesername] ) and ( type(mtoteser) == "function" or ((not stringIsLiteral[mtotesername]) and type(mtoteser) == "string") ) then
		...
		if type(mtoteser) == "function" then
			--Call the function
			a,b,c,d = mtoteser(info, ...)
		else
			--Call the method
			if handler and handler[mtoteser] then
				a,b,c,d = handler[mtoteser](handler, info, ...)
			else
				error(format("Method %s doesn't exist in handler for type %s", mtoteser, mtotesername))
			end
		end
		del(info)
		return a,b,c,d
		...
		for plugin, t in pairs(group.plugins) do
			for k, v in pairs(t) do
				if not opts[k] then
					...
					path[#path+1] = k
					tempOrders[k] = GetOptionsMtoteserValue("order", v, options, path, appName)
					tempNames[k] = GetOptionsMtoteserValue("name", v, options, path, appName)
					path[#path] = nil
				end
			end
		end
...
local function OptionOnMouseOver(widget, event)
	...
	local tooltipHyperlink = GetOptionsMtoteserValue("tooltipHyperlink", opt, options, path, appName)
	...
	local name = GetOptionsMtoteserValue("name", opt, options, path, appName)
	local desc = GetOptionsMtoteserValue("desc", opt, options, path, appName)
	local usage = GetOptionsMtoteserValue("usage", opt, options, path, appName)
...
local function CheckOptionHidden(option, options, path, appName)
	...
	return GetOptionsMtoteserValue("hidden", option, options, path, appName)
end
...
local function CheckOptionDisabled(option, options, path, appName)
	...
	return GetOptionsMtoteserValue("disabled", option, options, path, appName)
end
--[[
	...
	for i = 1, #keySort do
		...
		if v.type == "group" then
			...
			if not inline and not hidden then
				tinsert(tabs, k)
				text[k] = GetOptionsMtoteserValue("name", v, options, path, appName)
			end
			path[#path] = nil
		end
	end
...
local function BuildSelect(group, options, path, appName)
	...
	for i = 1, #keySort do
		...
		if v.type == "group" then
			...
			local hidden = CheckOptionHidden(v, options, path, appName)
			if not inline and not hidden then
				groups[k] = GetOptionsMtoteserValue("name", v, options, path, appName)
				tinsert(order, k)
			end
			path[#path] = nil
		end
	...
	for i = 1, #keySort do
		...
		if v.type == "group" then
			...
			if not inline and not hidden then
				local entry = new()
				entry.value = k
				entry.text = GetOptionsMtoteserValue("name", v, options, path, appName)
				entry.icon = GetOptionsMtoteserValue("icon", v, options, path, appName)
				entry.iconCoords = GetOptionsMtoteserValue("iconCoords", v, options, path, appName)
...
local function FeedOptions(appName, options,container,rootframe,path,group,inline)
	...
	for i = 1, #keySort do
		...
		tinsert(path, k)
		local hidden = CheckOptionHidden(v, options, path, appName)
		local name = GetOptionsMtoteserValue("name", v, options, path, appName)
		if not hidden then
			if v.type == "group" then
				...
				if v.type == "execute" then
					...
					local imageCoords = GetOptionsMtoteserValue("imageCoords",v, options, path, appName)
					local image, width, height = GetOptionsMtoteserValue("image",v, options, path, appName)
					...
					if iconControl then
						if not width then
							width = GetOptionsMtoteserValue("imageWidth",v, options, path, appName)
						end
						if not height then
							height = GetOptionsMtoteserValue("imageHeight",v, options, path, appName)
						end
				...
				elseif v.type == "input" then
					...
					local text = GetOptionsMtoteserValue("get",v, options, path, appName)
				...
				elseif v.type == "toggle" then
					...
					local value = GetOptionsMtoteserValue("get",v, options, path, appName)
					control:SetValue(value)
					control:SetCallback("OnValueChanged",ActivateControl)

					if v.descStyle == "inline" then
						local desc = GetOptionsMtoteserValue("desc", v, options, path, appName)
						control:SetDescription(desc)
					end

					local image = GetOptionsMtoteserValue("image", v, options, path, appName)
					local imageCoords = GetOptionsMtoteserValue("imageCoords", v, options, path, appName)
				...
				elseif v.type == "select" then
					local values = GetOptionsMtoteserValue("values", v, options, path, appName)
					local sorting = GetOptionsMtoteserValue("sorting", v, options, path, appName)
					if v.style == "radio" then
						local disabled = CheckOptionDisabled(v, options, path, appName)
						local width = GetOptionsMtoteserValue("width",v,options,path,appName)
						...
						control:PauseLayout()
						local optionValue = GetOptionsMtoteserValue("get",v, options, path, appName)
						if not sorting then
							sorting = {}
				...
				elseif v.type == "multiselect" then
					...
					if control then
						...
						for s = 1, #valuesort do
							...
							local value = GetOptionsMtoteserValue("get",v, options, path, appName, key)
						...
						for s = 1, #valuesort do
							...
							check:SetTriState(v.tristate)
							check:SetValue(GetOptionsMtoteserValue("get",v, options, path, appName, value))
				...
				elseif v.type == "color" then
					...
					control:SetLabel(name)
					control:SetHasAlpha(GetOptionsMtoteserValue("hasAlpha",v, options, path, appName))
					control:SetColor(GetOptionsMtoteserValue("get",v, options, path, appName))
					control:SetCallback("OnValueChanged",ActivateControl)
				...
				elseif v.type == "keybinding" then
					...
					control:SetLabel(name)
					control:SetKey(GetOptionsMtoteserValue("get",v, options, path, appName))
				...
				elseif v.type == "description" then
					...
					local fontSize = GetOptionsMtoteserValue("fontSize",v, options, path, appName)
					...
					local controlWidth = GetOptionsMtoteserValue("width",v,options,path,appName)
...
local function TreeOnButtonEnter(widget, event, uniquevalue, button)
	...
	local name = GetOptionsMtoteserValue("name", group, options, feedpath, appName)
	local desc = GetOptionsMtoteserValue("desc", group, options, feedpath, appName)
...
function AceConfigDialog:FeedGroup(appName,options,container,rootframe,path, isRoot)
	...
	if hasChildGroups and not inline then
		local name = GetOptionsMtoteserValue("name", group, options, path, appName)
...
function AceConfigDialog:Open(appName, container, ...)
	...
	local path = new()
	local name = GetOptionsMtoteserValue("name", options, options, path, appName)
	...
	if type(container) == "table" and container.type == "BlizOptionsGroup" and #path > 0 then
		...
		name = format("%s - %s", name, GetOptionsMtoteserValue("name", option, options, path, appName))
	end
